...
export const App = () => {
    ...
    const getNodeNameCN = (type: NodeType) => {
        switch (type) {
            case NodeType.PROMPT_INPUT: return '输入节点';
            case NodeType.IMAGE_GENERATOR: return '图像生成';
            case NodeType.VIDEO_GENERATOR: return '视频生成';
            case NodeType.IMAGE_TO_IMAGE: return '图生图（创意生图）';
            case NodeType.IMAGE_TO_VIDEO: return '图生视频';
            case NodeType.AUDIO_GENERATOR: return '音频生成';
    ...
    const getNodeIcon = (type: NodeType) => {
        switch (type) {
            case NodeType.PROMPT_INPUT: return <Type size={16} />;
            case NodeType.IMAGE_GENERATOR: return <ImageIcon size={16} />;
            case NodeType.VIDEO_GENERATOR: return <VideoIcon size={16} />;
            case NodeType.IMAGE_TO_IMAGE: return <Sparkles size={16} />;
            case NodeType.IMAGE_TO_VIDEO: return <VideoIcon size={16} />;
            case NodeType.AUDIO_GENERATOR: return <Mic2 size={16} />;
            case NodeType.VIDEO_ANALYZER: return <FileSearch size={16} />;
            case NodeType.IMAGE_EDITOR: return <Edit size={16} />;
    ...
    const addNode = useCallback((type: NodeType, x?: number, y?: number, initialData?: any) => {
        ...
        const typeMap: Record<NodeType, string> = {
            [NodeType.PROMPT_INPUT]: 'prompt', [NodeType.IMAGE_GENERATOR]: 'image', [NodeType.VIDEO_GENERATOR]: 'video',
            [NodeType.AUDIO_GENERATOR]: 'audio', [NodeType.VIDEO_ANALYZER]: 'video_analyzer',
            [NodeType.IMAGE_EDITOR]: 'image_editor', [NodeType.IMAGE_TO_IMAGE]: 'image_to_image', [NodeType.IMAGE_TO_VIDEO]: 'image_to_video'
        };
        ...
        const newNode: AppNode = {
            ...
            width: 420,
            title: getNodeNameCN(type) || '未命名节点',
    ...
    const handleNodeAction = useCallback(async (id: string, promptOverride?: string) => {
        ...
        try {
            ...
            inputs.forEach(n => { if (n?.data.image) imageInputs.push(n.data.image); });

            const isVideoOrImageToVideo = node.type === NodeType.VIDEO_GENERATOR || node.type === NodeType.IMAGE_TO_VIDEO;

            if (node.type === NodeType.IMAGE_GENERATOR || node.type === NodeType.IMAGE_TO_IMAGE) {
                if (!prompt && (!imageInputs || imageInputs.length === 0)) {
                    throw new Error(node.type === NodeType.IMAGE_TO_IMAGE ? '请输入创意描述或提供参考图像' : '请输入提示词');
                }
                ...
                if (imageInputs && imageInputs.length > 0) {
                    initialImage = imageInputs[0];
                    if (node.type === NodeType.IMAGE_TO_IMAGE) {
                        enhancedPrompt = prompt ? `${prompt} - 参考提供的图像风格和特征` : '请基于提供的图像直接生成风格相似或衍生的图像';
                    }
                }

                const model = node.data.model || 'gemini-2.5-flash-image';
                const aspectRatio = node.data.aspectRatio || '16:9';

                const isStoryboard = /分镜|storyboard|sequence|shots|frames|json/i.test(prompt);

                if (isStoryboard && node.type === NodeType.IMAGE_GENERATOR) { // Storyboard only for IMAGE_GENERATOR
                    try {
                ...
                if (!prompt && (!imageInputs || imageInputs.length === 0)) {
                    throw new Error(node.type === NodeType.IMAGE_TO_VIDEO ? '请输入创意描述或提供参考图像' : '请输入提示词');
                }
                ...
                // if (mode === 'CHARACTER' && characterImage) {
                ...
                const img = node.data.image || imageInputs[0];
                    ...
                    <div className="fixed z-[100] bg-[#1c1c1e]/80 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-1.5 min-w-[160px] animate-in fade-in zoom-in-95 duration-200 origin-top-left" style={{ top: contextMenu.y, left: contextMenu.x }} onMouseDown={(e) => e.stopPropagation()}>
                        ...
                        {contextMenuTarget?.type === 'create' && (
                            <>
                                ...
                                <button className="w-full px-4 py-2 text-left text-sm text-slate-200 hover:bg-cyan-500/20 hover:text-cyan-400 transition-colors flex items-center gap-3" onClick={() => { addNode(NodeType.IMAGE_GENERATOR, (contextMenu.x - pan.x) / scale, (contextMenu.y - pan.y) / scale); setContextMenu(null); }}><ImageIcon size={16} /> 图像生成</button>
                                <button className="w-full px-4 py-2 text-left text-sm text-slate-200 hover:bg-blue-500/20 hover:text-blue-400 transition-colors flex items-center gap-3" onClick={() => { addNode(NodeType.IMAGE_TO_IMAGE, (contextMenu.x - pan.x) / scale, (contextMenu.y - pan.y) / scale); setContextMenu(null); }}><Sparkles size={16} /> 图生图（创意）</button>
                                <button className="w-full px-4 py-2 text-left text-sm text-slate-200 hover:bg-purple-500/20 hover:text-purple-400 transition-colors flex items-center gap-3" onClick={() => { addNode(NodeType.VIDEO_GENERATOR, (contextMenu.x - pan.x) / scale, (contextMenu.y - pan.y) / scale); setContextMenu(null); }}><VideoIcon size={16} /> 视频生成</button>
                                <button className="w-full px-4 py-2 text-left text-sm text-slate-200 hover:bg-indigo-500/20 hover:text-indigo-400 transition-colors flex items-center gap-3" onClick={() => { addNode(NodeType.IMAGE_TO_VIDEO, (contextMenu.x - pan.x) / scale, (contextMenu.y - pan.y) / scale); setContextMenu(null); }}><VideoIcon size={16} /> 图生视频</button>
                                <button className="w-full px-4 py-2 text-left text-sm text-slate-200 hover:bg-pink-500/20 hover:text-pink-400 transition-colors flex items-center gap-3" onClick={() => { addNode(NodeType.AUDIO_GENERATOR, (contextMenu.x - pan.x) / scale, (contextMenu.y - pan.y) / scale); setContextMenu(null); }}><Mic2 size={16} /> 音频生成</button>

                                <div className="h-px bg-white/10 my-1"></div>

                                <div className="px-3 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 mt-1">处理与分析</div>
                                <button className="w-full px-4 py-2 text-left text-sm text-slate-200 hover:bg-emerald-500/20 hover:text-emerald-400 transition-colors flex items-center gap-3" onClick={() => { addNode(NodeType.VIDEO_ANALYZER, (contextMenu.x - pan.x) / scale, (contextMenu.y - pan.y) / scale); setContextMenu(null); }}><FileSearch size={16} /> 视频分析</button>
                                <button className="w-full px-4 py-2 text-left text-sm text-slate-200 hover:bg-rose-500/20 hover:text-rose-400 transition-colors flex items-center gap-3" onClick={() => { addNode(NodeType.IMAGE_EDITOR, (contextMenu.x - pan.x) / scale, (contextMenu.y - pan.y) / scale); setContextMenu(null); }}><Edit size={16} /> 图像编辑</button>
                            </>
                        )}
