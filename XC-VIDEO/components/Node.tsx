                ...
                <div className={`w-full rounded-[20px] p-1 flex flex-col gap-1 ${GLASS_PANEL} relative z-[100]`} onMouseDown={e => e.stopPropagation()} onWheel={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex items-center justify-between px-2 pb-1 pt-1 relative z-20">
                        <div className="flex items-center gap-2">
                            ...
                            {/* Dynamic Ratio Dropdown */}
                            {node.type !== NodeType.VIDEO_ANALYZER && node.type !== NodeType.AUDIO_GENERATOR && (
                                <div className="relative group/ratio">
                                    <div className="flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-white/5 cursor-pointer transition-colors text-[10px] font-bold text-slate-400 hover:text-cyan-400"><Scaling size={12} /><span>{node.data.aspectRatio || '16:9'}</span></div>
                                    <div className="absolute bottom-full left-0 pb-2 w-20 opacity-0 translate-y-2 pointer-events-none group-hover/ratio:opacity-100 group-hover/ratio:translate-y-0 group-hover/ratio:pointer-events-auto transition-all duration-200 z-[200]">
                                        <div className="bg-[#1c1c1e] border border-white/10 rounded-xl shadow-xl overflow-hidden">
                                            {(MODEL_CONFIGS[node.data.model!]?.ratios || (node.type.includes('VIDEO') ? VIDEO_ASPECT_RATIOS : IMAGE_ASPECT_RATIOS)).map(r => (
                                                <div key={r} onClick={() => handleAspectRatioSelect(r)} className={`px-3 py-2 text-[10px] font-bold cursor-pointer hover:bg-white/10 ${node.data.aspectRatio === r ? 'text-cyan-400 bg-white/5' : 'text-slate-400'}`}>{r}</div>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                            ...
                            {(node.type.includes('IMAGE') || node.type.includes('VIDEO_GENERATOR') || node.type === NodeType.IMAGE_TO_VIDEO) && (
                                <div className="relative group/resolution">
                                    ...
                                    <div className="absolute bottom-full left-0 pb-2 w-24 opacity-0 translate-y-2 pointer-events-none group-hover/resolution:opacity-100 group-hover/resolution:translate-y-0 group-hover/resolution:pointer-events-auto transition-all duration-200 z-[200]">
                                        <div className="bg-[#1c1c1e] border border-white/10 rounded-xl shadow-xl overflow-hidden">
                                            {node.data.model === 'kling-3.0' ? (
                                                MODEL_CONFIGS['kling-3.0'].qualities.map(q => (
                            ...
                            {(node.type === NodeType.VIDEO_GENERATOR || node.type === NodeType.IMAGE_TO_VIDEO) && MODEL_CONFIGS[node.data.model!]?.durations && (
                                <div className="relative group/duration">
                                    <div className="flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-white/5 cursor-pointer transition-colors text-[10px] font-bold text-slate-400 hover:text-cyan-400">
                                        <VideoIcon size={12} />
                                        <span>{node.data.duration || MODEL_CONFIGS[node.data.model!].durations[0]}s</span>
                                    </div>
                                    <div className="absolute bottom-full left-0 pb-2 w-16 opacity-0 translate-y-2 pointer-events-none group-hover/duration:opacity-100 group-hover/duration:translate-y-0 group-hover/duration:pointer-events-auto transition-all duration-200 z-[200]">
                                        <div className="bg-[#1c1c1e] border border-white/10 rounded-xl shadow-xl overflow-hidden">
                                            {MODEL_CONFIGS[node.data.model!].durations.map(d => (
                                                <div key={d} onClick={() => onUpdate(node.id, { duration: d })} className={`px-3 py-2 text-[10px] font-bold cursor-pointer hover:bg-white/10 ${(node.data.duration || MODEL_CONFIGS[node.data.model!].durations[0]) === d ? 'text-cyan-400 bg-white/5' : 'text-slate-400'}`}>{d}s</div>
                                            ))}
                                        </div>
                                    </div>
                                </div>
