...
import type { AgentTask, AgentTaskMetadata } from "../../../types/agent.types";
import type {
  CanvasElement,
  ChatMessage,
  InputBlock,
...
export function useWorkspaceSend(options: WorkspaceSendOptions) {
  ...
  return useCallback(
    async (
      ...
      const attachmentPreviews = allAttachmentFiles.map((file) => {
        const workspaceFile = file as WorkspaceInputFile;
        return workspaceFile._chipPreviewUrl || URL.createObjectURL(file);
      });
